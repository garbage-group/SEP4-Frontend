...
describe("HumidityDisplay Component", () => {
    ...
    test("renders input, button, and initial texts", () => {
        ...
        expect(screen.getByPlaceholderText(/Enter Bin Number/i)).toBeInTheDocument();
        expect(screen.getByRole("button", { name: /Get Humidity/i })).toBeInTheDocument();
    });
    ...
    test("displays error message on fetch failure", async () => {
        fetch.mockImplementationOnce(() => Promise.reject(new Error("Error fetching data")));
        render(<HumidityDisplay />);
        const button = screen.getByRole("button", { name: /Get Humidity/i });
